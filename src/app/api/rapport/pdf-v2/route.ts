...
import { renderReportHTMLV2 } from '@/lib/report/pdf-v2'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [bgCoverBase64, logoBase64] = await Promise.all([
      loadAssetBase64('bg_cover.png'),
      loadAssetBase64('logo_sceau_gold.png'),
    ])

    let html: string
    if (payloads.length === 1) {
      html = renderReportHTMLV2(firstPayload, bgCoverBase64, logoBase64)
    } else {
      const fullHtmls = payloads.map((p: typeof firstPayload) => renderReportHTMLV2(p, bgCoverBase64, logoBase64))
    ...
    const filename = `Rapport_Audit_V2_${modeLabel}_${agenceSlug}.pdf`
    ...
    return NextResponse.json(
      { error: 'PDF V2 generation failed', detail: String(err) },
      { status: 500 },
    )
  }
}
